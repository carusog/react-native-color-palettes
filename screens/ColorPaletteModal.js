...
export const ColorPaletteModal = ({ navigation }) => {
  ...
  const handleToggle = useCallback((isSelected, color) => {
    if (isSelected) {
      setSelectedColors((selectedColors) => [...selectedColors, color]);
    } else {
      setSelectedColors((palette) =>
        palette.filter((paletteColor) => paletteColor !== color),
      );
    }
  }, []);
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        style={[
          styles.textInput,
          isTextInputFocused ? styles.textInputFocus : null,
        ]}
        value={paletteName}
        placeholder="Enter the new palette name"
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ColorToggle color={item} onToggle={handleToggle} />
        )}
      />
...
const ColorToggle = ({ color, onToggle }) => {
  ...
  const textColor = {
    color:
      parseInt(color.hexCode.replace('#', ''), 16) > 0xffffff / 2.1
        ? 'black'
        : 'white',
  };
  return (
    <View style={[styles.colorItem, { backgroundColor: color.hexCode }]}>
      <Text style={[styles.colorItemText, textColor]}>{color.colorName}</Text>
      <Switch
        onValueChange={(newValue) => {
          toggleSwitch();
          onToggle(newValue, color);
        }}
        value={isEnabled}
      />
    </View>
...
const styles = StyleSheet.create({
  ...
  textInputFocus: { borderColor: 'skyblue' },
  button: {
